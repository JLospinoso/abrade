...
///
/// `authority` preserves the user-facing value. `host` is the name or address
/// passed to resolver/proxy negotiation, `service` is the textual port used by
/// Asio resolution, and `port` is the numeric TCP port used in SOCKS requests.
struct HostEndpoint {
...
///
/// Unbracketed IPv6 authorities keep the default service because a bare colon is
/// ambiguous. Use bracketed IPv6 syntax when an explicit port is required.
inline HostEndpoint parse_host_endpoint(std::string_view authority,
  ...
  return HostEndpoint{std::string{authority}, std::string{host}, std::string{service}, port};
}
} // namespace abrade

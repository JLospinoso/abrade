...
Options opt(const std::string& str) {
  ...
  const boost::tokenizer<boost::char_separator<char>> tokenizer{str,
                                                                boost::char_separator<char>{" "}};
  tokens.insert(tokens.end(), tokenizer.begin(), tokenizer.end());
  std::vector<const char*> cmdline{"abrade"};
  std::ranges::transform(tokens, back_inserter(cmdline),
...
TEST_CASE("Options") {
  ...
  SECTION("When given help") {
    ...
    SECTION("host is blank") { REQUIRE(options.get_host().empty()); }
  ...
  SECTION("Help string isn't blank") {
    auto options = opt("--help");
    REQUIRE_FALSE(options.get_help().empty());
  }
  ...
  SECTION("Parses host and pattern correctly with") {
    const char* const host_name = "lospi.net";
    const char* const pattern = "?asdf[1-10]";
  ...
  SECTION("Parses body filters correctly") {
    const auto cmdline =
        std::string{"lospi.net ?asdf[1-10] --contents --require FOUND --require PAGE "
                    "--reject SHELL --screen LEGACY --require-regex Race[0-9]+ "
                    "--reject-regex cancelled|postponed"};

    auto options = opt(cmdline);

    REQUIRE(options.get_required_literals() == std::vector<std::string>{"FOUND", "PAGE"});
    REQUIRE(options.get_rejected_literals() == std::vector<std::string>{"SHELL", "LEGACY"});
    REQUIRE(options.get_required_regexes() == std::vector<std::string>{"Race[0-9]+"});
    REQUIRE(options.get_rejected_regexes() == std::vector<std::string>{"cancelled|postponed"});
  }

  SECTION("Rejects body filters without contents") {
    const auto cmdline = std::string{"lospi.net ?asdf[1-10]"};

    REQUIRE_THROWS(opt(cmdline + " --require FOUND"));
    REQUIRE_THROWS(opt(cmdline + " --reject SHELL"));
    REQUIRE_THROWS(opt(cmdline + " --require-regex FOUND"));
    REQUIRE_THROWS(opt(cmdline + " --reject-regex SHELL"));
    REQUIRE_THROWS(opt(cmdline + " --screen LEGACY"));
  }

  SECTION("Rejects invalid regex filters during parsing") {
    const auto cmdline = std::string{"lospi.net ?asdf[1-10] --contents"};

    REQUIRE_THROWS(opt(cmdline + " --require-regex '['"));
    REQUIRE_THROWS(opt(cmdline + " --reject-regex '['"));
  }

  SECTION("Parses redirect options correctly") {
    const auto cmdline = std::string{"lospi.net ?asdf[1-10]"};

    SECTION("default") {
      auto options = opt(cmdline);
      REQUIRE(options.is_follow_redirects() == false);
      REQUIRE(options.get_max_redirects() == 5);
    }

    SECTION("with follow redirects") {
      auto options = opt(cmdline + " --follow-redirects --max-redirects 3");
      REQUIRE(options.is_follow_redirects() == true);
      REQUIRE(options.get_max_redirects() == 3);
    }

    SECTION("with invalid redirect values") {
      REQUIRE_THROWS(opt(cmdline + " --follow-redirects --max-redirects 0"));
      REQUIRE_THROWS(opt(cmdline + " --max-redirects 3"));
    }
  }
  ...
  SECTION("Parses proxy correctly with") {
    ...
    SECTION("default") {
      auto options = opt(cmdline);
      REQUIRE(options.get_proxy().empty());
  ...
  SECTION("Parses user agent correctly with") {
    ...
    SECTION("default") {
      auto options = opt(cmdline);
      REQUIRE(options.get_user_agent() ==
